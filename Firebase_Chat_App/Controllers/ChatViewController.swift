...
class ChatViewController: MessagesViewController {
    ...
    public static var dateFormatter: DateFormatter = {
        ...
        ft.dateFormat = "MM/dd/yyyy a HH:mm:ss Z"
//        ft.dateStyle = .medium
//        ft.timeStyle = .long
        ft.locale = Locale(identifier: "ko_KR")
        return ft
    }()
    ...
    private func presentLocationPicker() {
        ...
        vc.completion = { [weak self] selectedCoordinates in
            ...
            guard let messageId = self.createMessageId(), let conversationId = self.conversationId,
            ...
            DatabaseManager.shard.sendMessage(to: conversationId, otherUserEmail: self.otherUserEmail, name: name, newMessage: message) { success in
...
extension ChatViewController: InputBarAccessoryViewDelegate {
    ...
    func inputBar(_ inputBar: InputBarAccessoryView, didPressSendButtonWith text: String) {
        ...
        if isNewConversation {
            ...
            DatabaseManager.shard.createNewConversation(with: otherUserEmail, name: self.title ?? "User", firstMessage: message) { [weak self] success in
                if success {
                    print("message sent")
                    self?.isNewConversation = false
                    let newConversationId = "conversation_\(message.messageId)"
                    self?.conversationId = newConversationId
                    self?.listenForMessages(id: newConversationId, shouldScrollToBootom: true)
            ...
            DatabaseManager.shard.sendMessage(to: conversationId, otherUserEmail: otherUserEmail, name: name, newMessage: message) { [weak self] success in
                if success {
                    self?.messageInputBar.inputTextView.text = nil
                    print("message sent")
                }
...
extension ChatViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        guard let messageId = createMessageId(), let conversationId = conversationId,
        let name = self.title, let selfSender = selfSender else {
            return
        }
        ...
        if let image = info[.editedImage] as? UIImage, let imageData = image.pngData() {
            ...
            StorageManager.shard.uploadMessagePhoto(with: imageData, fileName: fileName) { [weak self] result in
                guard let self = self else { return }
            ...
            StorageManager.shard.uploadMessageVideo(with: videoUrl, fileName: fileName) { [weak self] result in
                guard let self = self else { return }
...
extension ChatViewController: MessagesDataSource, MessagesLayoutDelegate, MessagesDisplayDelegate {
    ...
    func configureAvatarView(_ avatarView: AvatarView, for message: MessageType, at indexPath: IndexPath, in messagesCollectionView: MessagesCollectionView) {
        ...
        if sender.senderId == selfSender?.senderId {
            ...
            // other user image
            if let otherUserPhotoURL = senderPhotoURL {
